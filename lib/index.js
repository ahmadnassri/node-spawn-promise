...
module.exports = function (cmd, args, opts, input) {
  return new Promise((resolve, reject) => {
    ...
    child.on('close', code => {
      stdout = [undefined, 'buffer'].indexOf(options.encoding) > -1 ? Buffer.concat(stdout) : stdout.join('')
      stderr = [undefined, 'buffer'].indexOf(options.encoding) > -1 ? Buffer.concat(stderr) : stderr.join('')
